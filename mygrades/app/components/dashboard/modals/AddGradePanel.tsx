...
import { Grade, useStudentStore } from '@/app/student/StudentStore'
...
const AddGradePanel = () => {
  ...
  const handleAddGrade = () => {
    ...
    if (targetSubject && newGrade) {
      addGrade(targetSubject.id, { ...newGrade, id: crypto.randomUUID() })
            ...
            onChange={(e) => setGradeValue(e.target.value)} onBlur={() => {
              ...
              if (!isNaN(num)) {
                if (Number(gradeValue) > 6) {
                  ...
                  setNewGrade({ ...newGrade, name: gradeName, value: num })
                  setCheckValue(true)
                }
              }
            }}
